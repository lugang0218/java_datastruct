...
public class NioClient3 implements Runnable {
    private Selector selector;
    private SocketChannel socketChannel;
    NioClient3(String ip, int port) {
        try {
            ...
            Connector connector=new Connector(socketChannel,selector);
            sk.attach(connector);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        NioClient3 nioClient=new NioClient3("localhost",8080);
        Thread clientThread=new Thread(nioClient);
        clientThread.start();
    }
}