...
public class BplusNode <K extends Comparable<K>, V> {
    ...
    /** 节点的关键字 */ 
    protected ArrayList<Map.Entry<K, V>> entries;
    /** 子节点 */ 
    ...
    public V search(K key){
        ...
        if(isLeaf){
            ...
            while(low<=high){
                ...
                else if(entries.get(mid).getKey().compareTo(key)>0){
                    high=mid-1;
                }
                else{
         ...
         * 如果不是叶子节点
         */
        //如果key比集合中第一个元素的key还要小 p[0]<k[0] key<k[0] 直接访问第一个子节点
         ...
         */
            ...
            while (low <= high) {
                int mid = (low + high) / 2;
                compareResult = entries.get(mid).getKey().compareTo(key);
                /**
                 *
                 *
                 * 如果key=K[i],也就是关键字等于K[i],
                 * 因为P[i]严格小于K[i]小于等于P[i+1]，
                 * 所以key对应的值一定实在P[i+1]中
                 */
                if (compareResult == 0) {
     ...
     *
     *
     * 插入节点之后父节点调整，看看是否需要分裂
     *
     * */
    protected void splitParent(BplusTree<K,V> tree){
        /**
         *
         * 如果子节点的数量超过order，就需要拆分
         */
